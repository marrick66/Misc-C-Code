...
//Compiler generated precursor code to <main>
08048320 <_start>:
...
0804841d <main>:
//Uses the cdecl calling convention.  Prior to calling a function,
//a new stack frame is created by saving the existing base pointer
//and then setting the base pointer to the current stack pointer.
//Then, any parameters or temp data is pushed to the stack.
 804841d:	55                   	push   ebp
 804841e:	89 e5                	mov    ebp,esp
//The stack is aligned on a multiple of 16.
 8048420:	83 e4 f0             	and    esp,0xfffffff0
//Stack space can be assigned by push instructions, or by subtracting
//the number of bytes from the stack pointer.
 8048423:	83 ec 20             	sub    esp,0x20
 8048426:	c7 44 24 1c 1e 00 00 	mov    DWORD PTR [esp+0x1c],0x1e
 804842d:	00 
 804842e:	8b 44 24 1c          	mov    eax,DWORD PTR [esp+0x1c]
//In the cdecl calling convention, the parameters are pushed right to left.
 8048432:	89 44 24 04          	mov    DWORD PTR [esp+0x4],eax		//This should be the value 30.
 8048436:	c7 04 24 e0 84 04 08 	mov    DWORD PTR [esp],0x80484e0	//This should be the address of the format string.
 804843d:	e8 ae fe ff ff       	call   80482f0 <printf@plt>
//In the cdecl calling convention, the return value is stored in eax.
 8048442:	b8 00 00 00 00       	mov    eax,0x0
//The stack can be reset by using the leave instruction, or by using the mov esp, ebp instruction.
 8048447:	c9                   	leave  
 8048448:	c3                   	ret    
